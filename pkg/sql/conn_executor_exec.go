...
func (ex *connExecutor) execStmtInOpenState(
	...
	if ex.executorType != executorTypeInternal &&
		...
		fmt.Printf("DEBUG-BEFORE-EMIT-RC: About to call emitMutationEvent for %T\n", stmt.AST)
		emitMutationEvent(stmtCtx, ex.server.cfg.EventEmitter, stmt.AST, p)
	} else {
		if err := ex.dispatchToExecutionEngine(stmtCtx, p, res); err != nil {
			stmtThresholdSpan.Finish()
			return nil, nil, err
		}
		fmt.Printf("DEBUG-BEFORE-EMIT: About to call emitMutationEvent for %T\n", stmt.AST)
		emitMutationEvent(stmtCtx, ex.server.cfg.EventEmitter, stmt.AST, p)
	}
	...
	if !os.ImplicitTxn.Get() && txn.IsSerializablePushAndRefreshNotPossible() {
		...
		fmt.Println("push detected for non-refreshable txn but auto-retry not possible")
	}
...
func (ex *connExecutor) execStmtInOpenStateWithPausablePortal(
	...
	if ex.executorType != executorTypeInternal &&
		...
		fmt.Printf("DEBUG-BEFORE-EMIT-2: About to call emitMutationEvent for %T\n", vars.stmt.AST)
		emitMutationEvent(stmtCtx, ex.server.cfg.EventEmitter, vars.stmt.AST, p)
	}
...
// emitMutationEvent emits table change event for reactive queries
func emitMutationEvent(ctx context.Context, emitter *EventEmitter, ast tree.Statement, p *planner) {
	fmt.Printf("REACTIVE: emitMutationEvent called, emitter=%v ast=%T\n", emitter != nil, ast)
	if emitter == nil {
		fmt.Printf("REACTIVE: emitter is nil\n")
		return
	}
	...
	switch n := ast.(type) {
	case *tree.Insert:
		tableName = extractTableNameFromExpr(n.Table)
		operation = "insert"
		primaryKey = extractPKFromInsert(ctx, n, p)
		fmt.Printf("DEBUG: INSERT table=%s pk=%v\n", tableName, primaryKey)
		fmt.Printf("REACTIVE: INSERT table=%s pk=%v", tableName, primaryKey)
	case *tree.Update:
		tableName = extractTableNameFromExpr(n.Table)
		operation = "update"
		primaryKey = extractPKFromUpdate(n)
		fmt.Println(ctx, 1, "REACTIVE: UPDATE table=%s pk=%v", tableName, primaryKey)
	case *tree.Delete:
		tableName = extractTableNameFromExpr(n.Table)
		operation = "delete"
		primaryKey = extractPKFromDelete(n)
		log.Dev.VEventf(ctx, 1, "REACTIVE: DELETE table=%s pk=%v", tableName, primaryKey)
	default:
		log.Dev.VEventf(ctx, 1, "REACTIVE: Not a mutation statement")
		return
	}

	if tableName != "" {
		log.Dev.VEventf(ctx, 1, "REACTIVE: Emitting event for table: %s", tableName)
		emitter.Emit(ctx, tableName, operation, primaryKey)
	} else {
		log.Dev.VEventf(ctx, 1, "REACTIVE: tableName is empty")
	}
}

// extractPKFromInsert extracts PK from INSERT using table descriptor
func extractPKFromInsert(ctx context.Context, n *tree.Insert, p *planner) map[string]string {
	pk := make(map[string]string)

	sel, ok := n.Rows.Select.(*tree.ValuesClause)
	if !ok || len(sel.Rows) == 0 {
		return pk
	}
	log.Dev.VEventf(ctx, 1, "REACTIVE: all values  %v rows", sel.Rows)
	firstRow := sel.Rows[0]

	// Check if values are placeholders - skip if so
	for _, val := range firstRow {
		valStr := val.String()
		if len(valStr) > 0 && valStr[0] == '$' {
			// Placeholder detected, skip PK extraction
			return pk
		}
	}

	// If columns specified, extract from those
	if len(n.Columns) > 0 {
		for i, col := range n.Columns {
			if i < len(firstRow) {
				if datum, ok := firstRow[i].(tree.Datum); ok {
					pk[col.String()] = datum.String()
				}
			}
		}
		return pk
	...
	// No columns specified - assume first value is "id" (simple fallback)
	if len(firstRow) > 0 {
		if datum, ok := firstRow[0].(tree.Datum); ok {
			pk["id"] = datum.String()
		}
	}
...
// extractPKFromUpdate extracts PK from UPDATE WHERE clause
func extractPKFromUpdate(n *tree.Update) map[string]string {
	pk := make(map[string]string)
	if n.Where != nil && n.Where.Expr != nil {
		extractPKFromExpr(n.Where.Expr, pk)
	}
	return pk
}

// extractPKFromDelete extracts PK from DELETE WHERE clause
func extractPKFromDelete(n *tree.Delete) map[string]string {
	pk := make(map[string]string)
	if n.Where != nil && n.Where.Expr != nil {
		extractPKFromExpr(n.Where.Expr, pk)
	}
...
// extractPKFromExpr extracts PK from WHERE expression
func extractPKFromExpr(expr tree.Expr, pk map[string]string) {
	switch e := expr.(type) {
	case *tree.ComparisonExpr:
		if e.Operator.Symbol.String() == "=" {
			if col, ok := e.Left.(*tree.ColumnItem); ok {
				if val, ok := e.Right.(tree.Datum); ok {
					pk[col.ColumnName.String()] = val.String()
				}
			}
		}
	case *tree.AndExpr:
		extractPKFromExpr(e.Left, pk)
		extractPKFromExpr(e.Right, pk)
	}
}

// extractTableNameFromExpr extracts table name from TableExpr
func extractTableNameFromExpr(expr tree.TableExpr) string {
	switch t := expr.(type) {
	case *tree.TableName:
		return strings.ToLower(t.Table())
	case *tree.AliasedTableExpr:
		return extractTableNameFromExpr(t.Expr)
	}
	return ""
}
